...
def compute_options(input_csv: str, output_csv: str | None = None) -> Path:
    ...
    results = df.apply(
        lambda row: black_scholes_call(
            S=row["spot"], K=row["strike"], T=row["T"], r=row["r"], sigma=row["sigma"]
        ),
        axis=1,
        result_type="expand",
    )
    ...
    if output_csv is None:
        output_path = input_path.with_name(f"{input_path.stem}_with_greeks.csv")
    else:
        output_path = Path(output_csv)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Compute Black-Scholes option prices and Greeks")
    parser.add_argument("input_csv", nargs="?", default="qqq_us_d_with_metrics.csv", help="Input CSV with synthetic option inputs")
